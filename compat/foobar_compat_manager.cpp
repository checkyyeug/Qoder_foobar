...
FoobarCompatManager::FoobarCompatManager()
...
Result FoobarCompatManager::initialize(const CompatConfig& config) {
...
void FoobarCompatManager::shutdown() {
    std::lock_guard<std::mutex> lock(mutex_);
...
FoobarDetectionResult FoobarCompatManager::detect_foobar2000() {
#ifdef _WIN32
...
FoobarDetectionResult FoobarCompatManager::detect_windows() {
...
FoobarDetectionResult FoobarCompatManager::detect_linux() {
    FoobarDetectionResult result;
...
FoobarDetectionResult FoobarCompatManager::detect_macos() {
    FoobarDetectionResult result;
...
bool FoobarCompatManager::validate_installation(const std::string& path) {
    if (path.empty()) {
        return false;
    }
...
Result FoobarCompatManager::init_adapters() {
...
Result FoobarCompatManager::update_config(const CompatConfig& config) {
    std::lock_guard<std::mutex> lock(mutex_);
...
Result FoobarCompatManager::register_adapter(std::unique_ptr<AdapterBase> adapter) {
    std::lock_guard<std::mutex> lock(mutex_);
...
Result FoobarCompatManager::scan_plugins(const std::string& directory) {
    std::lock_guard<std::mutex> lock(mutex_);
...
DataMigrationManager* FoobarCompatManager::get_migration_manager() {
    std::lock_guard<std::mutex> lock(mutex_);
    return migration_manager_.get();
}

void FoobarCompatManager::log(int level, const char* format, ...) {
    ...
    switch (level) {
        ...
        case 1: COMPAT_LOG_ERROR("[FoobarCompat] " + message); break;
        case 2: COMPAT_LOG_WARN("[FoobarCompat] " + message); break;
        case 3: COMPAT_LOG_DEBUG("[FoobarCompat] " + message); break;
        default: COMPAT_LOG_INFO("[FoobarCompat] " + message); break;
    }
}
