/**
 * @file qoder_plugin_sdk.h
 * @brief Qoder foobar 插件SDK - 核心接口定义
 * @date 2025-12-10
 */
...
// 平台相关定义
#ifdef _WIN32
    #ifdef QODER_PLUGIN_BUILD
        #define QODER_PLUGIN_API __declspec(dllexport)
    #else
        #define QODER_PLUGIN_API __declspec(dllimport)
    #endif
#else
    #define QODER_PLUGIN_API __attribute__((visibility("default")))
#endif
...
// 插件导出宏
#define QODER_DECLARE_PLUGIN(PluginClass, FactoryClass) \
    extern "C" { \
        QODER_PLUGIN_API qoder::IPluginFactory* qoder_create_plugin_factory() { \
            return new FactoryClass(); \
        } \
        QODER_PLUGIN_API const char* qoder_get_plugin_name() { \
...
#define QODER_EXPORT_AUDIO_PLUGIN(PluginClass) \
    QODER_DECLARE_PLUGIN(PluginClass, PluginClass##Factory)
