    ...
    if (decoder) {
        ...
        if (duration > 10) {
            ...
            if (decoder->get_info().name == "MP3 Decoder") {
                auto mp3_decoder = dynamic_cast<qoder::plugins::MP3Decoder*>(decoder.get());
                if (mp3_decoder) {
                ...
                auto flac_decoder = dynamic_cast<qoder::plugins::FLACDecoder*>(decoder.get());
                if (flac_decoder) {
                ...
                auto vorbis_decoder = dynamic_cast<qoder::plugins::OggVorbisDecoder*>(decoder.get());
                if (vorbis_decoder) {
...
int main(int argc, char* argv[]) {
    std::cout << "=== Qoder Audio Decoder Test ===" << std::endl;
