...
void ConfigManager::reset_to_defaults() {
    ...
    if (pw) {
        ...
        if (pw->pw_dir) {
            ...
            config_.config_file_path = std::string(pw->pw_dir) + "/.qoder-foobar/config.json";
        }
    }
}
...
bool ConfigManager::export_config(const std::string& file_path) const {
    try {
        ...
        json["exported_by"] = "Qoder foobar v" + config_.version;
...
void ConfigManager::load_from_environment() {
    // 音频配置
    if (const char* env = std::getenv("QODER_AUDIO_OUTPUT_DEVICE")) {
        config_.audio.output_device = env;
    }
    if (const char* env = std::getenv("QODER_SAMPLE_RATE")) {
        config_.audio.sample_rate = std::stoi(env);
    }
    if (const char* env = std::getenv("QODER_VOLUME")) {
        config_.audio.volume = std::stod(env);
    }

    // 插件配置
    if (const char* env = std::getenv("QODER_PLUGIN_DIR")) {
        config_.plugins.plugin_directories.clear();
        config_.plugins.plugin_directories.push_back(env);
    }

    // 播放器配置
    if (const char* env = std::getenv("QODER_MUSIC_DIR")) {
        config_.player.default_music_directory = env;
    }

    // 日志配置
    if (const char* env = std::getenv("QODER_LOG_LEVEL")) {
...
std::string ConfigManager::get_default_config_path() const {
    ...
    return std::string(pw->pw_dir) + "/.qoder-foobar/config.json";
}
