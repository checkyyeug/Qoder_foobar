...
Result CoreEngine::initialize() {
    ...
    // Initialize foobar2000 compatibility layer (if enabled)
    ...
    if (plugin_host_) {
        ...
        if (compat_service) {
            std::cout << "✓ foobar2000 compatibility service available" << std::endl;
        } else {
            std::cout << "⚠ foobar2000 compatibility service not loaded" << std::endl;
        }
    }
#endif
