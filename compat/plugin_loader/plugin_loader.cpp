...
const foobar2000_sdk::GUID& ServiceFactoryWrapper::get_guid() const {
    static const foobar2000_sdk::GUID null_guid = {0};
...
// FoobarPluginLoader 实现
FoobarPluginLoader::FoobarPluginLoader(FoobarCompatManager* compat_manager)
    : compat_manager_(compat_manager) {
}

FoobarPluginLoader::~FoobarPluginLoader() {
    unload_all();
}

#ifdef _WIN32
void* FoobarPluginLoader::load_library_internal(const char* path, std::string& error_msg) {
...
void FoobarPluginLoader::unload_library_internal(void* handle) {
    if (handle) {
...
Result FoobarPluginLoader::parse_client_entry(void* handle, PluginModuleInfo& module_info) {
    if (!handle) {
...
Result FoobarPluginLoader::enumerate_services(void* handle, std::vector<ServiceExportInfo>& services) {
    services.clear();
...
Result FoobarPluginLoader::register_services(const std::vector<ServiceExportInfo>& services) {
    if (!registry_bridge_) {
    ...
    for (const auto& service : services) {
        ...
        // 注册到服务注册表
        Result result = registry_bridge_->register_service(service.guid, wrapper.get());
...
bool FoobarPluginLoader::validate_abi_compatibility(void* handle) {
    if (!handle) return false;
    
    // 简单的 ABI 验证：检查必要导出是否存在
#ifdef _WIN32
    // 查找版本或初始化函数
    void* version_check = GetProcAddress(static_cast<HMODULE>(handle), "foobar2000_get_version");
    void* client_entry = GetProcAddress(static_cast<HMODULE>(handle), "_foobar2000_client_entry");
...
bool FoobarPluginLoader::validate_dependencies(const PluginModuleInfo& module_info) {
    (void)module_info;
    // 简化：假设所有依赖都满足
    // 真实实现会检查所需服务的可用性
    return true;
...
PluginModuleInfo FoobarPluginLoader::get_module_info(void* handle, const std::string& path) {
...
Result FoobarPluginLoader::load_plugin(const char* dll_path) {
...
Result FoobarPluginLoader::unload_plugin(const char* dll_path) {
    if (!dll_path || strlen(dll_path) == 0) {
        return Result::InvalidParameter;
    }
...
Result FoobarPluginLoader::load_plugins_from_directory(const char* directory) {
...
void FoobarPluginLoader::unload_all() {
    std::lock_guard<std::mutex> lock(mutex_);
...
const PluginModuleInfo* FoobarPluginLoader::get_module(const char* path) const {
...
bool FoobarPluginLoader::is_plugin_loaded(const char* path) const {
    return get_module(path) != nullptr;
}

void FoobarPluginLoader::set_registry_bridge(std::unique_ptr<ServiceRegistryBridge> bridge) {
    std::lock_guard<std::mutex> lock(mutex_);
