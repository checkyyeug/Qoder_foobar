...
PluginManager::PluginManager() : host_api_version_(QODER_PLUGIN_API_VERSION) {
}
...
bool PluginManager::load_native_plugin(const std::string& path) {
    ...
    auto create_factory = reinterpret_cast<IPluginFactory* (*)()>(
        GetProcAddress(handle, "qoder_create_plugin_factory"));
    if (!create_factory) {
    ...
    auto create_factory = reinterpret_cast<IPluginFactory* (*)()>(
        dlsym(handle, "qoder_create_plugin_factory"));
    if (!create_factory) {
        std::cerr << "Plugin does not export factory function: " << path << std::endl;
        dlclose(handle);
        return false;
